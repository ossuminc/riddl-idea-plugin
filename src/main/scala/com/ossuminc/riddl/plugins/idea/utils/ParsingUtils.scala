...
case class RiddlIdeaPluginLogger(numWindow: Int, override val withHighlighting: Boolean = true)
  extends Logger {
...
object ParsingUtils {
  ...
  def runCommandForWindow(numWindow: Int, confFile: Option[String] = None): Unit = {
    ...
    Commands.runCommandWithArgs(
      ...
      CommonOptions(noANSIMessages = false, groupMessagesByKind = true, showTimes = true)
    ) match {
      case Right(_) if windowState.getCommand == "from" => windowState.prependRunOutput("Success!! There were no errors found\n")
      case Left(_) => windowState.prependRunOutput("The following errors were found:\n")
      case _ => ()
    }
