...
import com.ossuminc.riddl.plugins.idea.files.utils.{getWholeWordsSubstrings, highlightKeywordsInDoc}
  ...
  override def documentChanged(event: DocumentEvent): Unit = {
    ...
    if doc != null then {
      ...
      if editors.nonEmpty then
        val newText = doc.getText(new TextRange(event.getOffset, event.getOffset + event.getNewLength))
        ...
        highlightKeywordsInDoc(
          wholeWords.zip(
            RiddlTokenizer.tokenize(wholeWords.map(_._1).mkString(" ")).filter(!_._1.isBlank)
